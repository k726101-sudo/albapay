...
class _AlbaMainScreenState extends State<AlbaMainScreen> {
  ...
  bool get _isEntryLandingLink {
    ...
    if (kDebugMode && isAttendanceAction) {
      print('DEBUG: _isEntryLandingLink identified as true (Action: attendance)');
    }
...
Future<void> _loadDashboardData() async {
  ...
  Future<void> _runEntryLandingAttendance() async {
    ...
    if (qrStoreId.isNotEmpty && qrStoreId != widget.storeId) {
      if (kDebugMode) print('DEBUG: StoreId mismatch: URL($qrStoreId) vs Widget(${widget.storeId})');
      return;
    }
    _autoHandled = true;
  ...
  void _showTopBanner(String message) {
    setState(() => _topBannerMessage = message);
  ...
  Future<void> _clockIn(Map<String, dynamic> worker, {bool isSilent = false}) async {
    ...
    try {
      // 1. Compliance check (Resilient - if it fails, we still try to record)
      List<Attendance> weekly = [];
      Store? store;
      try {
        weekly = await _weeklyAttendanceFuture();
        final storeSnap = await _db.collection('stores').doc(widget.storeId).get();
        store = Store.fromJson(storeSnap.data() ?? {});
        ...
        // We continue to allow recording the basic attendance even if compliance check fails
      }

      // 2. Unread notices check
      if (!isSilent) {
        try {
          final hasUnread = await _hasUnreadNotices();
          ...
          debugPrint('Notice check non-fatal error: $e');
        }
      }

      final todayStr = AppClock.now().toIso8601String().substring(0, 10);
      ...
      String? schedEndIso;
      late final DateTime payrollClockIn;
      ...
      if (shift == null) {
        ...
        payrollClockIn = payrollEffectiveClockIn(
          actualClockIn: now,
  ...
  Widget build(BuildContext context) {
    ...
    final worker = _workerData;
        final pages = <Widget>[
          ...
          AlbaPayrollPage(storeId: widget.storeId, workerId: _workerId, worker: worker),
