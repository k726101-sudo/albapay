...
class _LoginScreenState extends State<LoginScreen> {
  ...
  ({String? invite, String? storeId}) _readInviteParamsFromBrowser() {
    ...
    String? pick(Map<String, String> q) {
      ...
      if (a != null && a.isNotEmpty) return a.toLowerCase().startsWith('demo_') ? a : a.toUpperCase();
      final b = q['invite']?.trim();
      if (b != null && b.isNotEmpty) return b.toLowerCase().startsWith('demo_') ? b : b.toUpperCase();
      final c = q['code']?.trim();
      if (c != null && c.isNotEmpty) return c.toLowerCase().startsWith('demo_') ? c : c.toUpperCase();
      return null;
    }
  ...
  Future<void> _loginWithInviteAndPhone() async {
    ...
    try {
      ...
      // Step 2: Cloud Function(인사팀)에 초대 수락 요청
      // 서버가 어드민 권한으로 초대코드 검증 → 전화번호 매칭 → users/workers 업데이트를 안전하게 처리
      debugPrint('[LOGIN] Step 2: calling acceptInvite Cloud Function');
      ...
      debugPrint('[LOGIN] Step 2 done: result=${result.data}');
      ...
      debugPrint('[LOGIN] ✅ acceptInvite success: storeId=$storeId, workerId=$workerId');
      ...
      // Cloud Function(acceptInvite)에서 발생한 서버 에러
      debugPrint('[LOGIN] ❌ FirebaseFunctionsException: code=${e.code}, message=${e.message}');
  ...
  Future<void> _debugLoginAsTestWorker() async {
    ...
    try {
      ...
      // 3) workers 문서 등록 (고정 ID 사용) 및 서류/출결 데이터 생성
      final dummyWorkerData = {
        'id': DebugAuthConstants.testWorkerId,
        'name': '테스트 알바',
        'status': 'active',
        'storeId': storeId,
      ...
      await db
          ...
          .set(dummyWorkerData, SetOptions(merge: true));

      // 가상 직원의 출퇴근 기록 및 노무 서류 자동 생성
      await TestDataSeeder.generateVirtualWorkerAttendances(
        storeId: storeId,
        workersData: [dummyWorkerData],
      );
      if (!mounted) return;
