...
- `서비스 이용약관 및 개인정보 처리방침 확인` 버튼을 누르면 상세 약관 내역이 표시됩니다.
- 각 필수 약관에 체크(V)하신 후 **[동의하고 시작하기]**를 눌러주시면 기능이 활성화됩니다.
- 한 번 동의하신 이후에는 앱을 지울 때까지 다시 묻지 않습니다.
...
> [!TIP]
> **아직 가입하기 망설여지시나요? "앱 체험하기" 모드를 활용해보세요!**
> 화면 최하단의 `[앱 체험하기 (가상 데이터로 시작)]` 버튼을 누르면 회원가입 없이 가상의 매장과 직원 데이터(5인)가 자동으로 세팅되어 실제 급여 정산과 노무 서류 관리 기능을 100% 동일하게 미리 경험해 보실 수 있습니다!

---

## 2. 사업장 정보 등록 (처음 1회 필수)
...
1. **상시 근로자 수 설정 (가장 중요!)**
   - **자동 스캔(추천)**: 앱에 등록된 직원들의 근무표를 바탕으로 5인 이상/미만 여부를 시스템이 매주 자동으로 계산합니다.
   - 근로기준법상 5인 이상과 미만은 **연장수당, 야간/휴일수당, 연차휴가 발생** 등 급여 계산 로직에 결정적인 영향을 미치므로 가급적 [자동 스캔]을 권장합니다.
2. **급여 정산일**
   - **정산 산정 기간 (시작일 ~ 종료일)**: (예) 1일부터 말일까지.
   - **급여 지급일**: 실제 직원들에게 통장 입금하는 날.
   - 위 설정에 따라 매월 급여 명세서가 자동으로 잘려서 산출됩니다.
3. **주휴 수당 표시 여부**
...
> [!IMPORTANT]
> **사업자 등록증 촬영 및 정보 연동**
> 노무 서류(계약서류 등) 하단에는 사장님의 사업자등록번호와 서명이 들어갑니다. [서류 등록/촬영] 버튼을 통해 명확한 정보를 등록해 두시면 서류 자동 완성에 큰 도움이 됩니다.

---

## 3. 신규 직원 등록

급여 정산의 핵심인 직원 근무표를 등록하는 화면입니다. 여러 부가 기능들(사회보험, 보건증)이 숨겨져 있어 상황에 맞게 펼쳐서 설정할 수 있습니다.

<div style="page-break-inside: avoid; text-align: center;">
  <img src="images/add_staff_top.png" alt="직원 등록 상단" style="width: 23%; border: 1px solid #e0e0e0; border-radius: 8px; margin: 0 3px;" />
  <img src="images/add_staff_bottom.png" alt="직원 등록 하단" style="width: 23%; border: 1px solid #e0e0e0; border-radius: 8px; margin: 0 3px;" />
  <img src="images/screen_3.png" alt="사회보험 설정 펼침" style="width: 23%; border: 1px solid #e0e0e0; border-radius: 8px; margin: 0 3px;" />
  <img src="images/screen_4.png" alt="보건증 설정 펼침" style="width: 23%; border: 1px solid #e0e0e0; border-radius: 8px; margin: 0 3px;" />
</div>

### 🔹 기본 입력 정보
1. **직원 정보**: 이름과 연락처를 남겨주세요. 이후 직원이 폰에서 본인용 파트너 앱(웹링크)으로 접속하려면 연락처 기반 초대 링크 발송이 필요합니다.
2. **급여 및 요일/근무시간 (매우 중요)**
   - **시급**: 2026년 기준 법정 최저임금은 **10,320원**입니다. 기본값이 자동으로 안내됩니다.
   - **휴게 여부**: 유급 휴게인지 무급 휴게인지 명확하게 구분해주세요. (통상적으로 식사시간 등은 무급 휴게입니다.)
   - 💡 **주휴수당 자동 판독**: 요일 아이콘 버튼들을 터치하여 출근 요일을 켜고 시간을 지정하면, 합산된 '주 소정근로시간'을 바탕으로 **주휴수당 달성 요건(15시간 이상 여부)**을 시스템이 자동으로 알려줍니다.

### 🔹 고급 설정 창 (접기/펼치기)
앱 하단에 있는 각 항목의 헤더를 탭하면 세부 설정 화면이 아래로 확장됩니다.

1. **4대보험(사회보험) 설정**
   - 국민연금, 건강보험, 고용보험 가입 여부를 개별적으로 세팅할 수 있습니다.
   - 단시간 근로자, 또는 일용직 근로자의 경우 법정 요건에 맞추어 버튼을 켜주시면, 매월 급여 정산 화면에서 **근로자 부담분이 자동으로 계산되어 급여에서 공제**됩니다.
2. **보건증(건강진단결과서) 관리**
   - 요식업 등 필수 직군의 경우 보건증 발급일과 유효기간(통상 1년) 만료일을 등록할 수 있습니다.
   - 정보를 미리 입력해두시면 만료 전(예: 한 달 전) 대시보드 대관 화면에서 알맞게 푸시 알림 혹은 강력한 경고를 발생시켜 줍니다. 과태료를 완벽히 예방하세요!

> [!TIP]
> 화면 최하단의 **[초대 코드 보내고 등록 완료]** 버튼을 누르시면, 직원이 카카오톡 등의 채널로 본인의 앱 접속용 초대 링크를 전송받게 되면서 즉시 출퇴근 기록 모드로 진입하게 됩니다!
...
노동부 점검 시 단골 과태료 대상이 되는 각종 법정 서류들을 더 이상 수기로 작성할 필요가 없습니다. 앱에 입력된 매장 및 직원 정보를 바탕으로 PDF 문서 형태로 완벽하게 자동 렌더링됩니다.
...
- 대시보드의 **[노무 서류 확인]** 탭에 들어가면 사장님이 발송한 근로계약서 팝업이 노출됩니다.
...
매장 운영과 관련해 사장님이 즉각적으로 인지해야 하는 중요 알림들이 카드뷰 형태로 펼쳐집니다. 
- **긴급/주의 알림**: 보건증 만료 임박, 근로계약서 미작성 경고 등 당장 처리해야 할 컴플라이언스 이슈를 알려줍니다.
- **할 일 목록**: 오늘 출근할 직원이 누구인지, 서명이 필요한 결재류가 있는지 등을 스마트하게 요약하여 브리핑해줍니다.
- 마지막 슬라이드인 **[급여 브리핑/리포트]** 카드를 통해 이번 달 인건비와 주휴수당 예상치를 대시보드 위젯 상에서도 빠르게 요약 형태로 확인할 수 있습니다.
- 해당 카드를 터치하면 즉시 관련 상세 메뉴로 이동하여 신속한 조치가 가능합니다.

### 🔹 실시간 출퇴근 관리
가장 눈에 잘 띄는 곳에 현재 매장에서 실시간으로 일하고 있는 직원의 명단과 누적 근무 시간이 투명하게 공개됩니다.
- 알바생이 본인의 기기(알바웹)에서 출근 버튼을 누르는 즉시 사장님 화면에도 실시간으로 타이머가 돌아갑니다.
- 지각 여부나 휴게 시간 사용 유무도 이 화면에서 최우선으로 파악하실 수 있습니다.
...
- 사장님이 매번 "이번달 얼마야?" 하는 질문에 답할 필요 없이 직원이 스스로 납득하고 언제나 조회가 가능합니다.
...
- 사장님이 앱에서 급하게 일정을 변경하더라도 직원 웹 캘린더에 실시간으로 즉시 반영됩니다.
...
- 사장님이 별도로 종이 명세서를 뽑아주지 않아도, 앱 캡처 한 장으로 급여 명세서 교부 의무를 갈음할 수 있는 강력한 기능입니다.
