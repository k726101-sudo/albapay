...
exports.cleanupNotificationLogs = exports.sendNoticePush = exports.sendAttendanceCreatedPush = exports.sendAttendancePush = exports.processNotificationQueue = exports.sendDocumentStatusPush = exports.sendSpecialClausePush = exports.acceptInvite = exports.cleanupDemoSandbox = exports.sendSubstitutionPush = exports.appleSignInAndroidBridge = void 0;
...
exports.acceptInvite = (0, https_1.onCall)({
    ...
    if (!inviteCode || typeof inviteCode !== "string" ||
        ...
        throw new https_1.HttpsError("invalid-argument", "초대코드와 전화번호를 입력해 주세요.");
    }
    ...
    if (normalizedPhone.length < 10 || normalizedPhone.length > 11) {
        throw new https_1.HttpsError("invalid-argument", "유효한 전화번호를 입력해 주세요.");
    }
    const db = admin.firestore();
    // ── 3. invite 문서 조회 (Transaction 밖에서 읽기 → storeId/workerId 확보) ──
    let inviteDocRef = null;
    let storeId = null;
    let workerIdHint = null;
    ...
    if (!storeId) {
        firebase_functions_1.logger.warn("acceptInvite: invite not found", { callerUid, inviteCode: normalizedInvite });
        throw new https_1.HttpsError("not-found", "초대 링크가 유효하지 않습니다.");
    }
    ...
    if (!matchedWorkerId) {
        ...
        if (workerSnap.empty) {
            firebase_functions_1.logger.warn("acceptInvite: no active worker for invite", { callerUid, storeId, inviteCode: normalizedInvite });
            throw new https_1.HttpsError("not-found", "초대코드가 유효하지 않습니다.");
        }
    ...
    if (!matchedWorkerId) {
        firebase_functions_1.logger.warn("acceptInvite: phone mismatch", { callerUid, storeId, phone: normalizedPhone });
        throw new https_1.HttpsError("not-found", "사장님이 등록하지 않은 번호입니다. 매장에 문의하세요.");
    }
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (!workerDoc.exists) {
            throw new https_1.HttpsError("not-found", "직원 정보를 찾을 수 없습니다.");
        }
        const workerData = workerDoc.data();
        const existingUid = workerData.uid?.toString().trim() || "";
        // 하이재킹 방지: 다른 uid가 이미 점유 → 거부
        if (existingUid && existingUid !== callerUid) {
            throw new https_1.HttpsError("already-exists", "이미 다른 사용자가 등록된 직원입니다. 매장에 문의하세요.");
        }
        ...
        if (inviteDocRef) {
            ...
            if (inviteDoc.exists) {
                ...
                if (usedAt && usedByUid && usedByUid !== callerUid) {
                    ...
                    throw new https_1.HttpsError("already-exists", "이미 사용된 초대코드입니다. 매장에 새 초대코드를 요청해 주세요.");
                }
    ...
    firebase_functions_1.logger.info("acceptInvite success", {
        callerUid,
...
exports.sendDocumentStatusPush = (0, firestore_1.onDocumentUpdated)("stores/{storeId}/documents/{docId}", async (event) => {
    const before = event.data?.before.data();
    const after = event.data?.after.data();
    if (!before || !after)
        return;
    ...
    if (!workerId)
        return;
    let docTypeStr = "문서";
    if (after.type === "laborContract")
        docTypeStr = "근로계약서";
    else if (after.type === "wageStatement")
        docTypeStr = "임금명세서";
    else if (after.type === "nightHolidayConsent")
        docTypeStr = "야간/휴일 근로 동의서";
    else if (after.type === "employeeRegistry")
        docTypeStr = "근로자명부";
    else if (after.type === "parentalConsent")
        docTypeStr = "친권자 동의서";
    else if (after.type === "pledge")
        docTypeStr = "서약서";
    ...
    if (before.status === "draft" && after.status === "sent") {
        ...
        try {
            await admin.messaging().send(payload);
            firebase_functions_1.logger.info("Document sent push successful", { topic, storeId });
        }
        catch (error) {
            firebase_functions_1.logger.error("Failed to send document sent push", error);
        }
    }
    ...
    if (before.status === "sent" && after.status === "delivered") {
        ...
        try {
            await admin.messaging().send(payload);
            firebase_functions_1.logger.info("Document delivered push successful", { topic, storeId });
        }
        catch (error) {
            firebase_functions_1.logger.error("Failed to send document delivered push", error);
        }
    }
});
// ============================================================================
...
function collapseKeyForType(type, storeId) {
...
async function checkRateLimit(targetUid) {
...
exports.processNotificationQueue = (0, firestore_1.onDocumentCreated)({
    ...
    try {
        if (channel === "pushBoss" && targetUid) {
            ...
            if (!allowed) {
                firebase_functions_1.logger.warn("Rate limit exceeded", { targetUid, docId: event.params.docId });
            ...
            if (tokens.length === 0 && storeId) {
                ...
                batchResult.responses.forEach((resp, idx) => {
                    if (!resp.success) {
                        ...
                        if (errCode === "messaging/registration-token-not-registered" ||
                            ...
                            firebase_functions_1.logger.warn("Removing invalid FCM token", { token: tokens[idx].substring(0, 10), uid: targetUid });
                        }
                    }
                });
                ...
                if (batchResult.successCount === 0 && storeId) {
                    ...
                    firebase_functions_1.logger.warn("All FCM tokens failed, wrote in-app alert", { targetUid });
                }
            }
        }
        ...
        firebase_functions_1.logger.info("Notification sent", { docId: event.params.docId, channel, type });
    }
    catch (error) {
        const errMsg = error instanceof Error ? error.message : String(error);
        firebase_functions_1.logger.error("Notification send failed", { docId: event.params.docId, error: errMsg, retryCount });
...
// ============================================================================
exports.sendAttendancePush = (0, firestore_1.onDocumentUpdated)({
    ...
    if (!wasClockOut && isClockOut) {
        const status = after.attendanceStatus;
        ...
        if (status && abnormalStatuses.includes(status)) {
            ...
            try {
                ...
                firebase_functions_1.logger.info("Attendance push sent (clockOut)", { storeId, status });
            }
            catch (error) {
                firebase_functions_1.logger.error("Attendance push failed", error);
            }
        }
    }
});
...
// ============================================================================
exports.sendAttendanceCreatedPush = (0, firestore_1.onDocumentCreated)({
    document: "attendance/{attendanceId}",
    region: "asia-northeast3", // #5 서울 리전
}, async (event) => {
    const data = event.data?.data();
    ...
    try {
        ...
        firebase_functions_1.logger.info("Attendance created push sent", { storeId, status });
    }
    catch (error) {
        firebase_functions_1.logger.error("Attendance created push failed", error);
    }
});
// ============================================================================
...
// ============================================================================
exports.sendNoticePush = (0, firestore_1.onDocumentCreated)({
    ...
    try {
        await admin.messaging().send(payload);
        firebase_functions_1.logger.info("Notice push sent", { topic, storeId, noticeId: event.params.noticeId });
    }
    catch (error) {
        firebase_functions_1.logger.error("Failed to send notice push", error);
    }
});
// ============================================================================
...
// ============================================================================
exports.cleanupNotificationLogs = (0, scheduler_1.onSchedule)({
    ...
    if (logDeleted > 0) {
        ...
        firebase_functions_1.logger.info(`Cleaned up ${logDeleted} old notification logs`);
    }
    ...
    for (const status of ["sent", "failed", "rate_limited"]) {
        ...
        if (qDeleted > 0) {
            await qBatch.commit();
            firebase_functions_1.logger.info(`Cleaned up ${qDeleted} old queue docs (status=${status})`);
        }
    }
});
// R2 Storage Controller (generateUploadUrl, finalizeUpload, generateDownloadUrl)
__exportStar(require("./storage/r2_controller"), exports);
