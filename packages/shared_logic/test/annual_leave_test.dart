  ...
  group('Annual Leave Calculation Accuracy Tests', () {
    ...
    test('1년 미만 매달 만근 시 연차 1개씩 정상 적립 (최대 11개)', () {
      ...
      for (int m = 0; m < 11; m++) {
        ...
        final mEnd = DateTime(2023, 1 + m + 1, 1).subtract(const Duration(days: 1));
        
        for (var d = mStart; !d.isAfter(mEnd); d = d.add(const Duration(days: 1))) {
      ...
      expect(summary.calculationBasis.any((s) => s.contains('2023-11: 만근 (+1개)')), isTrue);
    });
    ...
    test('1년 시점 80% 이상 출근 시 15개 추가 부여 확인 (총 11+15=26개)', () {
      ...
      expect(summary.calculationBasis.any((s) => s.contains('출근율 100.0%')), isTrue);
    });
    ...
    test('관리자 수동 조정값 반영 확인', () {
      ...
      expect(summary.calculationBasis.any((s) => s.contains('관리자 수동 조정: +3.5개')), isTrue);
    });
