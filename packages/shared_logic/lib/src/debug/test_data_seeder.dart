...
class TestDataSeeder {
  ...
  static const _inviteCodes = ['TST001', 'TST002', 'TST003', 'TST004', 'TST005'];
  ...
  Future<void> seed({required String storeId}) async {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      final workerData = <String, dynamic>{
        ...
        'phone': i == 0 ? '01000000001' : '0100000${(1000 + i).toString().padLeft(4, '0')}',
      ...
      batch.set(
        _db.collection('invites').doc(invite),
        {
          'storeId': storeId,
          'workerId': id,
          'staffName': '테스트알바${i + 1}',
    ...
    for (var i = 0; i < 20; i++) {
      ...
      final att = Attendance(
        ...
        scheduledShiftStartIso: DateTime(day.year, day.month, day.day, 9, 0)
            .toIso8601String(),
        scheduledShiftEndIso: DateTime(day.year, day.month, day.day, 18, 0)
            .toIso8601String(),
  ...
  static Future<void> generateVirtualWorkerAttendances({
    ...
    if (workersData != null && workersData.isNotEmpty) {
      ...
      targetWorkers = workersSnap.docs.map((d) => {...d.data(), 'id': d.id}).toList();
    }

    final startDay = now.subtract(const Duration(days: 40));
    var batch = db.batch();
    var ops = 0;
    ...
    for (var worker in targetWorkers) {
      ...
      for (int i = 0; i <= 40; i++) {
        ...
        // 오늘보다 미래의 날짜는 제외
        if (DateTime(d.year, d.month, d.day).isAfter(DateTime(now.year, now.month, now.day))) continue;
        ...
        final shortStoreId = storeId.length > 10 ? storeId.substring(storeId.length - 8) : storeId;
        final attId = 'dbg_val_${workerId}_${d.toIso8601String().substring(0, 10)}_$shortStoreId';
        ...
        final att = Attendance(
          ...
          storeId: storeId,
          clockIn: inDt,
          clockOut: isCurrentlyWorking ? null : outDt, // 오늘이고 첫번째 직원이면 퇴근 안함 (당일 근무중 표시)
          originalClockIn: inDt,
          originalClockOut: isCurrentlyWorking ? null : outDt,
          isAutoApproved: true,
          type: AttendanceType.web,
          attendanceStatus: isCurrentlyWorking ? 'Working' : 'Normal',
        ...
        if (!isCurrentlyWorking) attJson['originalClockOut'] = outDt.toIso8601String();
      ...
      batch.set(
        db.collection('stores').doc(storeId).collection('workers').doc(workerId),
      ...
      for (var dt in docTypes) {
        ...
        batch.set(
          db.collection('stores').doc(storeId).collection('documents').doc(docId),
          {
            'id': docId,
            'staffId': workerId,
            'storeId': storeId,
            ...
            'signatureUrl': isSigned ? 'https://dummyimage.com/200x100/000/fff&text=Worker+Sign' : null,
            'bossSignatureUrl': isSigned ? 'https://dummyimage.com/200x100/000/fff&text=Boss+Sign' : null,
          },
          SetOptions(merge: true)
        );
        ops++;
    ...
    try {
      ...
      if (uid != null) {
        await db.collection('users').doc(uid).update({
          'seederDebug': 'Success: $ops ops. TargetWorkers: ${targetWorkers.length}. First worker: ${targetWorkers.isNotEmpty ? targetWorkers.first['id'] : "none"}'
        });
      }
    } catch (_) {}
  }
