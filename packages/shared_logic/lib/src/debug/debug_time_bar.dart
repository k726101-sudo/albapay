                        ...
                        GestureDetector(
                          onTap: () async {
                            ...
                            await TestDataSeeder.generateVirtualWorkerAttendances(
                                storeId: DebugAuthConstants.debugStoreId);
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('★ 한 달 치 가상 출퇴근 데이터 생성 완료!')),
                              );
                            }
                          },
                          child: const Text('🤖 1달치 자동출근',
                              style: TextStyle(
                                  fontSize: 12,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.indigo)),
                    ...
                    Row(
                      children: [
                        ...
                        _buildBtn(Icons.remove, () {
                          ...
                          AppClock.setDebugOverride(t.subtract(const Duration(days: 1)));
                        }),
                        _buildBtn(Icons.add, () {
                          debugPrint('[Debug] Plus day');
                          AppClock.setDebugOverride(t.add(const Duration(days: 1)));
                        }),
                        const SizedBox(width: 12),
                        _buildLabel('시간'),
                        _buildBtn(Icons.remove, () {
                          debugPrint('[Debug] Minus hour');
                          AppClock.setDebugOverride(t.subtract(const Duration(hours: 1)));
                        }),
                        Text('${t.hour}시', style: const TextStyle(fontWeight: FontWeight.bold)),
                        _buildBtn(Icons.add, () {
                          debugPrint('[Debug] Plus hour');
                          AppClock.setDebugOverride(t.add(const Duration(hours: 1)));
                        }),
                        const SizedBox(width: 12),
                        _buildLabel('분'),
                        _buildBtn(Icons.remove, () {
                          debugPrint('[Debug] Minus 10m');
                          AppClock.setDebugOverride(t.subtract(const Duration(minutes: 10)));
                        }),
                        Text('${t.minute}분', style: const TextStyle(fontWeight: FontWeight.bold)),
                        _buildBtn(Icons.add, () {
                          debugPrint('[Debug] Plus 10m');
                          AppClock.setDebugOverride(t.add(const Duration(minutes: 10)));
                        }),
                        const SizedBox(width: 16),
                        GestureDetector(
                          onTap: () => AppClock.setDebugOverride(null),
                          child: const Text('실제 시간',
